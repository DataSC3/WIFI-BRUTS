...
🎯 Supported Platforms
Flipper Zero (with WiFi DevBoard or external module)

ESP32-based devices

Other platforms with compatible JavaScript environment and WiFi capabilities

✨ Features
📡 Network Scanner: Automatically discovers nearby WiFi networks with signal strength indicators

📊 Visual Interface: Clean menu system with scrolling support and progress indicators
...
📈 Real-time Progress: Live progress bar with current password attempts

⏹️ Attack Control: Stop attacks at any time with button press

🔍 Network Filtering: Automatically filters for WPA2-PSK and WEP encrypted networks

💾 Serial Logging: All attempts logged to serial for analysis

🛠️ Installation & Usage
Load the Script: Deploy the script to your compatible device

Select Target: Use the scanner to find target networks

Choose Dictionary: Load existing wordlist or generate birthday-based passwords

Launch Attack: Monitor progress in real-time

Review Results: Successful finds are displayed immediately
...
Custom Lists: Plain text files with one password per line
...
Flipper Zero (с WiFi DevBoard или внешним модулем)
...
✨ Возможности
📡 Сканер сетей: Автоматически обнаруживает nearby WiFi сети с индикацией силы сигнала

📊 Визуальный интерфейс: Чистая система меню с поддержкой прокрутки и индикаторами прогресса
...
📈 Прогресс в реальном времени: Индикатор выполнения с отображением текущих попыток

⏹️ Управление атакой: Остановка атаки в любой момент

🔍 Фильтрация сетей: Автоматическая фильтрация WPA2-PSK и WEP сетей

💾 Логирование: Все попытки логируются в serial для анализа

🛠️ Установка и использование
Загрузка скрипта: Разверните скрипт на совместимом устройстве

Выбор цели: Используйте сканер для поиска целевых сетей

Выбор словаря: Загрузите существующий wordlist или сгенерируйте пароли на основе дат рождения

Запуск атаки: Мониторьте прогресс в реальном времени

Просмотр результатов: Найденные пароли отображаются немедленно
...
Пользовательские списки: Текстовые файлы с одним паролем на строку
